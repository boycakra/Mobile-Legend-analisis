...
togglePasswordButton.addEventListener("click", () => {
    if (passwordInput.type === "password") {
	  ...
	  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
		<path stroke-linecap="round" stroke-linejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C0 13.658.126 15.508.756 17.258l6.764 6.764a1.2 1.2 0 001.707 0l6.764-6.764a1.2 1.2 0 000-1.707l-6.764-6.764A10.477 10.477 0 001.934 8.223z" />
	  </svg>
	`;
    } else {
        ...
        togglePasswordButton.innerHTML = `
	  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
		<path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 0 .39 1.618l7.494 7.494a1.2 1.2 0 0 0 1.707.39c1.523 0 2.755-.8 3.037-2.08a1.2 1.2 0 0 0-.39-1.708l-7.494-7.494z" />
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("Kill").addEventListener("click", function () {
        if (marking && currentPlayer && currentMove) {
            ...
            valueDisplays[
                currentPlayer
            ].innerHTML = `Value: Kill ${currentMove}`;
        }
    });
    ...
    document.getElementById("assisted").addEventListener("click", function () {
        if (marking && currentPlayer && currentMove) {
            ...
            valueDisplays[
                currentPlayer
            ].innerHTML = `Value: assisted ${currentMove}`;
        }
    });
    ...
    document.getElementById("Death").addEventListener("click", function () {
        if (marking && currentPlayer && currentMove) {
            ...
            valueDisplays[
                currentPlayer
            ].innerHTML = `Value: Death ${currentMove}`;
    ...
    document
        .getElementById("Buff Merah")
        .addEventListener("click", function () {
            if (marking && currentPlayer && currentMove) {
                markingCircleClass = `${currentMove}Buff-Merah-mark-circle`;
                valueDisplays[
                    currentPlayer
                ].innerHTML = `Value: Inner Tower Destroy ${currentMove}`;
                const time = timerDisplay.textContent;
                addDataToTable(time, currentPlayer, currentMove);
            }
        });
    document.getElementById("Buff biru").addEventListener("click", function () {
        if (marking && currentPlayer && currentMove) {
            ...
            valueDisplays[
                currentPlayer
            ].innerHTML = `Value: Base Tower Destroy ${currentMove}`;
            const time = timerDisplay.textContent;
            addDataToTable(time, currentPlayer, currentMove);
        }
    });
    ...
    document.getElementById("Lord").addEventListener("click", function () {
        if (marking && currentPlayer && currentMove) {
            ...
            valueDisplays[
                currentPlayer
            ].innerHTML = `Value: Lord ${currentMove}`;
            const time = timerDisplay.textContent;
            addDataToTable(time, currentPlayer, currentMove);
        }
    });
    ...
    document.getElementById("Turtle").addEventListener("click", function () {
        if (marking && currentPlayer && currentMove) {
            ...
            valueDisplays[
                currentPlayer
            ].innerHTML = `Value: turtle ${currentMove}`;
            const time = timerDisplay.textContent;
            addDataToTable(time, currentPlayer, currentMove);
        }
    });
    ...
    function removeTableEntry(mark, tableBody) {
        const rowIndex = Array.from(tableBody.children).findIndex(row => {
            ...
            return (
                cellCoordinates === mark.coordinates && cellTime === mark.time
            );
        });
        ...
        switch (key) {
            ...
            case "F":
                ...
                if (marking && currentPlayer && currentMove) {
                    ...
                    valueDisplays[
                        currentPlayer
                    ].innerHTML = `Value: Buff Merah ${currentMove}`;
                }
                break;
            case "g":
            case "G":
            ...
            case "H":
                ...
                if (marking && currentPlayer && currentMove) {
                    ...
                    valueDisplays[
                        currentPlayer
                    ].innerHTML = `Value: Buff-biru ${currentMove}`;
                }
                break;
            case "j":
            case "J":
